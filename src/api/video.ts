...
http.interceptors.response.use(
  ...
  (error) => {
    if (error.code === 'ECONNABORTED') {
      console.warn('[VibeTV] Request timed out')
    }
...
export async function testLineApi(line: LineConfig): Promise<boolean> {
  try {
    ...
    console.log(`[VibeTV] Line ${line.name} is ${isOnline ? 'online' : 'offline'}`)
    return isOnline
  } catch (error: any) {
    console.error(`[VibeTV] Test failed for ${line.name}:`, error?.message || error)
    return false
  }
}
...
export async function getHomeList(line: LineConfig): Promise<VideoItem[]> {
  try {
    ...
    if (data?.list && Array.isArray(data.list)) {
      console.log(`[VibeTV] Found ${data.list.length} items`)
      ...
      if (itemsWithoutCovers.length > 0) {
        console.log(`[VibeTV] ${itemsWithoutCovers.length} items missing covers, trying batch detail fetch`)
    ...
    console.error(`[VibeTV] Get home list failed for ${line.name}:`, error?.message || error)
    return []
  }
}
...
async function fetchMissingCovers(items: VideoItem[], line: LineConfig): Promise<void> {
  try {
    ...
    console.warn(`[VibeTV] Failed to fetch covers:`, error)
  }
}
...
export async function searchVideos(keyword: string, line: LineConfig): Promise<{ list: VideoItem[] }> {
  try {
    ...
    console.error(`[VibeTV] Search failed for ${line.name}:`, error?.message || error)
    return { list: [] }
  }
}
...
export async function getVideoDetail(id: string, line: LineConfig): Promise<{ item: VideoItem; sources: VideoSource[]; playSources: VideoPlaySource[] } | null> {
  try {
    ...
    console.error(`[VibeTV] Get detail failed for ${line.name}:`, error?.message || error)
    return null
  }
}
