  ...
  >
    <div class="container mx-auto px-4">
      <div class="flex items-center justify-between h-16 md:h-20">
        <div class="flex items-center gap-2 md:gap-8 cursor-pointer" @click="goHome">
          ...
          <div class="hidden sm:block">
            <span class="text-xl md:text-2xl font-bold tracking-wide" style="font-family: 'Playfair Display', serif; background: linear-gradient(135deg, #e50914 0%, #f5af19 50%, #e50914 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; text-shadow: 0 0 30px rgba(229, 9, 20, 0.3);">
              VibeLume
            </span>
        ...
        <nav class="hidden lg:flex items-center gap-1">
          ...
          <div class="relative ml-4">
            <button
              class="px-4 py-1.5 rounded-lg bg-dark-card text-gray-300 hover:bg-dark-hover transition-colors text-sm flex items-center gap-1"
              @click="showSourceMenu = !showSourceMenu"
            >
              <span>{{ appStore.activeLine?.name || '精选影视' }}</span>
            ...
            <div
              v-if="showSourceMenu"
              class="absolute right-0 mt-2 w-44 bg-dark-card rounded-lg shadow-xl border border-gray-800 py-1 z-50 animate-scale-in"
            >
              <button
